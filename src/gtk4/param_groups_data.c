/*****************************************************************************
 *
 * gtk4/param_groups_data.c
 *
 * PHASEX:  [P]hase [H]armonic [A]dvanced [S]ynthesis [EX]periment
 *
 * Copy of gui_layout.c's init_param_groups()/init_param_pages() and the
 * param_group[]/param_page[]/*_order[] data they populate -- pure data,
 * no GTK calls, so it's safe to reuse as-is. Can't link gui_layout.c
 * itself: its OTHER functions (create_param_group(), etc.) call
 * gtk_table_new()/gtk_hbox_new()/gtk_vbox_new(), which don't exist in
 * GTK4, so that file doesn't compile under GTK4 headers at all.
 *
